...
class Prueba extends Command
{
     ...
     * @var string
     */
    protected $signature = 'transacciones:prueba';
                ...
                if(strtolower($g->frecuencia->descripcion) == "diario"){
                    ...
                    if($fecha->hour == 14){
                       //Verificamos que no haya transacciones realizadas en la fecha actual para la banca y el gasto especificado
                        if(transactions::where(['idBanca' => $b['id'], 'idGasto' => $g->id, 'status' => 1])->whereBetween('created_at', array($fechaDesde, $fechaHasta))->first() != null)
                            continue;
